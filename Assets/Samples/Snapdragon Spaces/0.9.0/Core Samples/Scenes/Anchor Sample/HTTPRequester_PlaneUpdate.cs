...
public class HTTPRequester_PlaneUpdate : MonoBehaviour
{
    ...
    public void Post()
    {
        RestClient.Post<ReturnCheckIfNewMail>(baseURL + "check", new Post { }).Then(response => {
            ...
            {
                Debug.Log("here has_new");
                Functions.hasNew = true;
                // call instantiate paper plane
                paperPlane.FlyToParkingLot();
                ...
                RestClient.Post<ReturnReadUnreadMail>(baseURL + "unread", new ReadUnreadMail
                {
                    ...
                    var msg = JObject.Parse(res.ToString())["latest_preview"];
                    Functions.latestMail = msg.ToString();
                    Debug.Log(Functions.latestMail);
                    });
            }
            else{
                Functions.hasNew = false;
            }
        });
    }
}